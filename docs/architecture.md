...
8. **Recovery pass.** When verification comes back weak, scoped re-sourcing queries look for better citations, and the new material is verified and merged in. The anti-fabrication prompt asks for primary/authoritative sources, but this is prompt-only — there's no forced domain allowlist, since that previously injected generic HIGH-tier citations into off-topic reports. Instead, each recovered result is relevance-gated against the claim it was recovered for (content-word overlap or a literal figure match). See [`../src/polysearch/verification/recovery.py`](../src/polysearch/verification/recovery.py).
...
10. **Report.** The orchestrator totals cost and duration and writes a markdown report plus a sibling JSON. The "Sources by Quality Tier" section lists only sources actually attributed to a checked claim (within claim-verifiable layers); merely-collected-but-uncited sources collapse into a one-line count note instead of a full listing.
